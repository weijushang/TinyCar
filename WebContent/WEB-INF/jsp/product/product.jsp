  ...
  <body>
	...
	<div id="carousel-example-generic" class="carousel slide" data-ride="carousel">
        <ol class="carousel-indicators">
          ...
          <c:forEach items="${product_images}" varStatus="status" var="item">
        ...
        <c:if test="${product_images!=null && fn:length(product_images)>0 }">
        ...
        <table class="table">
          ...
          <tr>
            <td style="vertical-align:middle;">
            	...
            	<a href="<%=request.getContextPath()%>/car/product_detail.htm?product_id=${item.product_id }">
            	<img src="<%=CarConfig.getString(Constant.IMAGE_URL) %>${item.file_path}" alt="" width="112px" height="58px">
            	</a>
            	</div>
            </td>
            <td>
            	<h5><a href="<%=request.getContextPath()%>/car/product_detail.htm?product_id=${item.product_id }">${item.product_title }</a></h5>
            	<table border=0 width="100%">
            		<tr>
            			<td>${fn:substring(item.update_time,0,10) }</td>
            			<td align="right"><button type="button" class="btn btn-primary btn-xs" onclick="window.location.href='<%=request.getContextPath()%>/car/credit_expert.htm?product_id=${item.product_id}'">贷款专家</button></td>
            		</tr>
            	</table>
            </td>
          </tr>
